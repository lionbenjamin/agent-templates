---
description: Autonomous pipeline from idea to working code — specify, clarify, plan, review, tasks, implement, code review, security review, QA. No manual gates. Skips stages already completed.
---
...
27. Run `/security_agent` for a full ASVS 5.0 verification (Level 2, all 17 chapters). The review writes its report to `docs/*/security/security-review.md`.
28. **If verdict is NEEDS WORK**:
    - Read every 🚨 Critical and ⚠️ High finding from the report.
    - Fix each finding directly in the code.
    - For Medium/Low findings: apply them if they are quick wins; otherwise leave them on the remediation roadmap.
    - Re-run `/security_agent` to confirm the code now passes.
    - Loop until the verdict is PASS (max 3 iterations — if still failing, stop and ask the user).
29. Output: "Stage 8 complete — security review passed (ASVS 5.0 L2)."

---

### Stage 9: Quality Assurance + Fix
30. Run `/quality` for comprehensive QA — edge case testing, correctness verification, and code quality audit.
31. **If verdict is NOT PASS**:
    ...
    - Loop until the verdict is PASS (max 3 iterations — if still failing, stop and ask the user).
32. Output: "Stage 9 complete — QA passed."

---

### Stage 10: Summary
33. Present a final summary to the user:
